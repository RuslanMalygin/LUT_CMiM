...
%% Solve constraint equation using NR for position and velocity +acceleration
C_fun = @(t, q) constraint(revolute, simple, driving, t, q);
Cq_fun = @(t, q) constraint_dq(revolute, simple, driving, t, q);
...
plot(Q(:, 4), Q(:, 5), ...
    ...
    0, 0, '*', 'LineWidth', 2);
axis equal
title('Position of slider crank')
...
plot(QP(:, 4), QP(:, 5), ...
    ...
    0, 0, '*', 'LineWidth', 2);
axis equal
title('Velosity of slider crank')
%% Some verification plots
figure
plot(QPP(:, 4), QPP(:, 5), ...
    QPP(:, 7), QPP(:, 8), ...
    ...
    0, 0, '*', 'LineWidth', 2);
axis equal
title('Acceleration of slider crank')